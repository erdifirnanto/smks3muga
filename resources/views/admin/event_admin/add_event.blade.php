      ...
      <form id="validate" class="user" method="POST" action="{{route('store.event_admin')}}" enctype="multipart/form-data">
        ...
        <div class="form-group row">
          ...
          <div hidden class="col-sm-6">
            ...
            <input type="text" class="form-control form-control-lg @error('pembuat') is-invalid @enderror" id="pembuat" placeholder="Pembuat" name="pembuat" value="{{ old('pembuat', Auth::user()->name) }}">
            @error('pembuat')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
